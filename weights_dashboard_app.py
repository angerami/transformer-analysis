...
def load_weight_data(model_name, weight_name):
    return load_group_from_file(f"histos/{model_name}.{weight_name}.histos.pkl")

st.title("Transformer Weight Analysis")

# Sidebar
model_display_name = st.sidebar.selectbox("Model", list(model_display.keys()))
model = model_display[model_display_name]
weight_type = st.sidebar.selectbox("Weight", ["W_q", "W_k", "W_v", "W_QK"])

# Load data
hgroup_obj = load_weight_data(model, weight_type)
hgroup = hgroup_obj.histogroup
for _, hbase in hgroup_obj:
    extract_metrics_(hbase)

n_layers = max(k[0] for k in hgroup.keys()) + 1
n_heads = max(k[1] for k in hgroup.keys()) + 1
...
hb = hgroup[layer, head]
h_dict = hb.histograms

# Plot selector with display names
available_plots = {k: v for k, v in plot_display.items() if v in h_dict}
plot_display_name = st.selectbox("Plot", list(available_plots.keys()))
plot_type = available_plots[plot_display_name]
...
fig = go.Figure()
y_vals = np.where(h > 0, np.log10(h), pmin) if use_log_1 else h
fig.add_trace(go.Bar(x=h_centers, y=y_vals, name=plot_display_name))

if show_fit and 'fit_normal' in hb.stats_values:
    mu = hb.stats_values['fit_normal']['fit_mu']
    sigma = hb.stats_values['fit_normal']['fit_sigma']
    # Gaussian curve
...
col1.metric("Mean", f"{hb.get_statistic('mean'):.4f}")
col2.metric("Std", f"{hb.get_statistic('std'):.4f}")
col3.metric("Max", f"{hb.get_statistic('max'):.4f}")
...
# # Compute stat array
stats = np.zeros((n_layers, n_heads))
for (l, h), val in hgroup.items():
    stats[l, h] = val.get_statistic(stat_name)
