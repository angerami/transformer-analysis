...
def model_sweep(
    model_list, out_dir="Drive/ana-002", clobber=False, cache_dir="./downloads"
):
    ...
    for model_name in tqdm(model_list):
        ...
        if os.path.exists(target_dir):
            if clobber:
                print(
                    f"Model = {model_name} output exists as {target_dir}. OVERWRITING."
                )
            else:
...
if __name__ == "__main__":
    ...
    model_sweep(
        model_list=models, out_dir=out_dir, clobber=args.clobber, cache_dir=args.cache
    )
